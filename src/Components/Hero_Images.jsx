...
import heroGirlImage from "../assets/Images/1.png";

const Hero_Images = () => {
  const heroImages = [mainImage, cloudyImage, heroGirlImage];
  return (
    <Fragment>
      <div className="Hero-Div w-full h-screen relative overflow-hidden">
            ...
            <React.Fragment key={index}>
              <img
                ...
                className={`object-cover absolute z-20 ${
                  index < 1 && "z-30 w-full"
                } ${index < 2 && "top-0 left-0 w-full h-full"} ${
                  index === 2 &&
                  "left-1/2 -translate-1/2 top-[90%] z-40 scale-[1.2] w-fit"
                }`}
              />
            </React.Fragment>
          );
